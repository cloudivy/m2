...
st.title("🔬 Probability of Stress Corrosion - CHAKSU MATHURA SECTION")
...
if uploaded_file is not None:
    try:
        ...
        required_cols = ['Wd (ID)', 'OFF PSP (VE V)', 'conductivity', 'Hoop stress% of SMYS', 'Stationing (m)']
        missing_cols = [col for col in required_cols if col not in df_scc_II.columns]
        if missing_cols:
            st.error(f"Missing columns: {missing_cols}. Check dataset.")
        else:
            # Normalize and compute (your exact logic)
            ...
            feature_weights_normalized = {
                'conductivity': 0.186, 'Hoop stress% of SMYS': 0.08,
                'Normalized_Distance_from_Pump(KM)': 0.165, 'Inverse_Normalized_OFF_PSP_VE_V': 0.142
            }
            ...
            ax.set_title('Normalized Stress Corrosion Probability Score vs. Stationing (m) - CHAKSU MATHURA')
